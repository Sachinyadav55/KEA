...
%%
% mas5 data
load drug_mas5_NCI60_huex1.0.mat
...
%%    
 ...
 %%     
    ...
    n_feature = 0; step = 500;
    while n_feature < size(x,2)

        %train on all training data and sort the features
...
%   save file
    ...
    [~,Ii]=sort(y_drug); Ii = 1:size(x,1);
    for i=1:size(x,1)
        fprintf(f,'%s %.4f %.4f %.4f\n', name{Ii(i)}, y_drug(Ii(i)), score(Ii(i)), z_drug(Ii(i)));
    end
    fclose(f);
