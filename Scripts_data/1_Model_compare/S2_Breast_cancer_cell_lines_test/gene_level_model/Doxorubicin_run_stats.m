...
%%
% mas5 data
load drug_mas5_reduced_extended_gene.mat
...
NBT_raw=importdata('DREAM7_DrugSensitivity1_GeneExpression.txt');
%%
nbt_marker=NBT_raw.textdata(2:end,1);
%%
...
%%
    ...
    mean_x=mean(mean(x));
    ltemp=size(NBT_rawdata);
    mx=zeros(ltemp(1),1)+mean_x;
...
%%
    ...
    sorted_feature = [];
%%    
    NBT_Y_raw=importdata('NBT_ic50_gene.tsv');
...
%%
    ...
    %predict on NBT
    for f_i=1:I
        temp_f=find(ismember(nbt_marker,feature_name(f_i)) );
        if(isempty(temp_f))
    ...
    b=mean(y_drug);
    a=1;
    rd_add=normrnd(0,1,1,length(yin))*0.2*yrange;
...
%   save file
    ...
    ln=sum(isnan(NBT_label));
    test_acc=(1-(nansum(abs(plabel-NBT_label))+7)/(length(plabel)-10))*100;
%%
    ...
    legend('Sensitive', 'Resistant', 'Not determined','breast cancer cells','Location','bestoutside');
