...
DrugResponse.predict <- function(patient_inputfile,inputfile_formate,patient_name,path.to.model='../data'){
    ...
    drug1_f=file(paste(path.to.model,"prob/svm4_Topotecan.txt",sep="/"))
    drug1_prob= read.table(drug1_f)
    drug2_f=file(paste(path.to.model,"prob/svm4_Gemcitabine.txt",sep="/"))
    drug2_prob=read.table(drug2_f)
    drug3_f=file(paste(path.to.model,"prob/svm4_Gefitinib.txt",sep="/"))
    drug3_prob= read.table(drug3_f)
    drug4_f=file(paste(path.to.model,"prob/svm4_Cisplatin.txt",sep="/"))
    drug4_prob= read.table(drug4_f)
    drug5_f=file(paste(path.to.model,"prob/svm4_Doxorubicin.txt",sep="/"))
    drug5_prob= read.table(drug5_f)
    drug6_f=file(paste(path.to.model,"prob/svm4_Erlotinib.txt",sep="/"))
    drug6_prob= read.table(drug6_f)
    drug7_f=file(paste(path.to.model,"prob/svm4_Docetaxel.txt",sep="/"))
    drug7_prob= read.table(drug7_f)
    drug8_f=file(paste(path.to.model,"prob/svm4_Paclitaxel.txt",sep="/"))
    drug8_prob= read.table(drug8_f)
    drug9_f=file(paste(path.to.model,"prob/svm4_Carboplatin.txt",sep="/"))
    drug9_prob= read.table(drug9_f)

    drug1wb_f=file(paste(path.to.model,"for_Rtest/Topotecan_wbx.txt",sep="/"))
    drug1wb_prob=read.table(drug1wb_f)    
    drug2wb_f=file(paste(path.to.model,"for_Rtest/Gemcitabine_wbx.txt",sep="/"))
    drug2wb_prob=read.table(drug2wb_f)
    drug3wb_f=file(paste(path.to.model,"for_Rtest/Gefitinib_wbx.txt",sep="/"))
    drug3wb_prob= read.table(drug3wb_f)
    drug4wb_f=file(paste(path.to.model,"for_Rtest/Cisplatin_wbx.txt",sep="/"))
    drug4wb_prob= read.table(drug4wb_f)
    drug5wb_f=file(paste(path.to.model,"for_Rtest/Doxorubicin_wbx.txt",sep="/"))
    drug5wb_prob= read.table(drug5wb_f)
    drug6wb_f=file(paste(path.to.model,"for_Rtest/Erlotinib_wbx.txt",sep="/"))
    drug6wb_prob= read.table(drug6wb_f)
    drug7wb_f=file(paste(path.to.model,"for_Rtest/Docetaxel_wbx.txt",sep="/"))
    drug7wb_prob= read.table(drug7wb_f)
    drug8wb_f=file(paste(path.to.model,"for_Rtest/Paclitaxel_wbx.txt",sep="/"))
    drug8wb_prob= read.table(drug8wb_f)
    drug9wb_f=file(paste(path.to.model,"for_Rtest/Carboplatin_wbx.txt",sep="/"))
    drug9wb_prob= read.table(drug9wb_f)
    
    drug_name=cbind('Topotecan','Gemcitabine','Gefitinib','Cisplatin','Doxorubicin','Erlotinib','Docetaxel','Paclitaxel','Carboplatin')
    ...
    for(i in 1:n_p)
    {
		...
		t_score[i,8]=DrugResponse.score(i, drug8wb_prob, drug8_prob, test_geneExp, test_normalized, t_d, drug_name[8], test_geneExp_all, test_normalized_all)
		t_score[i,9]=DrugResponse.score(i, drug9wb_prob, drug9_prob, test_geneExp, test_normalized, t_d, drug_name[9], test_geneExp_all, test_normalized_all)
